<template>
    <div  id="TopNav"  class="fixed bg-white z-30 flex items-center w-full border-b h-[61px]">
        <div :class="route.fullPath === '/' ? 'max-w-[1150px]' : ''"  class="flex items-center justify-between w-full px-6 mx-auto">
            <div :class="route.fullPath === '/' ? 'w-[80%]' : 'lg:w-[20%] w-[70%]'">
                <NuxtLink to="/" >
                    <img width="115" src="~/assets/images/tiktok-logo.png">
                </NuxtLink>
            </div>
            <div class="hidden md:flex items-center bg-white p-1 rounded-full max-width-[380] w-full">
                <input
                    type="text" 
                    class="w-full pl-3 my-2 bg-transparent placeholder-[#838383] text-[15px] focus:outline-none"
                    placeholder="Look for accounts"
                />   
                <div class="px-3 py-1 flex items-center border-l border-l-gray-300">
                    <Icon name="el:search-alt" color="#A1A2A7" size="22"/>
                </div>  
                <div class="flex items-center justify-end gap-3 min-w-[275] mx-w[320] w-full">
                    <button class="flex items-center border rounded-sm px-3 py-[6px] hover:bg-gay-100">
                        <Icon  name="+" color="#0000" size="22"/>
                        <span class="px-2 font-meduim text-[15px]">Upload</span>
                    </button>
                    <div v-if="!$userStore.id" class="flex items-center" >
                        <button 
                        @click="$event => $generaleStore.isLoginOpen = true"
                        class="flex items-center bg-[#F02C56] text-white border rounded-md px-3 py-[6py]">
                            <span class="mx-4  font-meduim text-[15px]">Login</span>
                        </button>
                        <Icon name="mdi:dots-vertical" color="#161724" size="25"/>
                    </div>
                    <div v-else class="flex items-center">
                        <Icon class="ml-1 mr-4" name="carbon:send-alt" color="#161724" size="30"/>
                        <Icon class="mr-5" name="bx:message-detail" color="#161724" size="27"/>
                        <div class="relative">
                            <button @click="$event=>showMenu=!showMenu">
                                <img  class="rounded-full"  width="33"  
                                src="https://fastly.picsum.photos/id/83/300/320.jpg?hmac=l9ZpmvOptn3rVS5tziPy5BPu72V9tbNc1O8mF1eW3E0">
                            </button>
                            <div
                                v-if="showMenu"
                                id="PopupMenu"
                                class="absolute bg-white rounded-lg py-1.5  w-[200px] shadow-xl border top-[43px] -right-2">

                            <NuxtLink
                                @click="$event=>showMenu = false"
                                class="flex items-center justify-start py-3 px-2 px-2 hover:bg-gray-100 cursor-pointer"
                                >
                                <Icon name="ph:user" size="20"/>
                                <span class="pl-2 font-semibold text-sm">Profil</span>
                            </NuxtLink>
                            <div
                                class="flex items-center justify-start py-3 px-1.5 px-2 border-t hover:bg-gray-100 cursor-pointer"
                                >
                                <Icon name="ic:outline-login" size="20"/>
                                <span class="pl-2 font-semibold text-sm">Logout</span>
                            </div>
                            </div>
                        </div>
                    </div>

                </div>
                
            </div>

        </div>
    </div>
</template>

<script setup>
const route = useRoute()
let showMenu = ref(false)
const {$generaleStore}=useNuxtApp();

</script>